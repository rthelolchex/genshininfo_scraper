    ...
    try {
        ...
        let retry = function() {
            ...
            if (data.retcode === 10101) {
                throw new Error("Cannnot get data for more than 30 accounts per cookie per day.")
            }
            if (data.retcode === 10001) {
                throw new Error('Login cookies have not been provided or incorrect.')
            }
            if (data.retcode === 10102) {
                throw new Error("User\'s data is not public, please try again!")
            }
            if (data.retcode === -100) {
                throw new Error('Login cookies have not been provided or are incorrect.')
            }
            if (data.retcode === 1009) {
                throw new Error("Could not find the user, uid may not valid")
            }
            if (data.retcode === -108) {
                throw new Error("Language invalid!")
            }
            if (data.retcode === 10103) {
                throw new Error ("Not logged in, please regenerate the cookie token!")
            }
            if (data.retcode === -10001) {
...
exports.spiralAbyss = async(cookie, uid, schedule) => {
    ...
    try {
        ...
        let retry = function() {
            ...
            if (data.retcode === -1) {
                throw new Error("Invalid schedule type, please fill 1 for today, 2 for last week")
            }
        }
        do {
            console.log("Attempting to login...", "Attempt :", attempt)
...
${result.precious_chest_number} Precious chest unlocked
