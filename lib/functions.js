 ...
 */
 exports.getDailyRewardInfo = async (cookie) => {
    const { data: html } = axios.get(OS_URL + "info", {
        headers: {
            'cookie': cookie,
            'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.97 Safari/537.36',
        },
 ...
 */
 exports.simpleCharacterList = async(data) => {
    if (data.data === null) return data.message
...
exports.simpleSpiralAbyss = async(data) => {
    if (data.data === null) return data.message
    let res = data.data
    if (data.data.max_floor === '0-0') return "No spiral abyss data."
    if (!res.max_floor) return "This is not a spiral abyss data!"
    let str = `
